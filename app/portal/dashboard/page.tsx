...
import { LogOut, BookOpen, FileText, CheckCircle2, TrendingUp, Calendar as CalIcon } from "lucide-react";

export default async function StudentDashboard() {
  // 🔥 THE DEEP FIX: Await the cookies promise before trying to 'get' them
  const cookieStore = await cookies(); 
  
  const studentId = cookieStore.get("student_id")?.value;
  const batchName = cookieStore.get("student_batch")?.value;
  const instituteId = cookieStore.get("institute_id")?.value;

  // Security Check: If no cookie is found, kick them back to login
  ...
  // Fetch their specific batch materials and test scores (Limit to latest 3)
    ...
    <div className="min-h-screen bg-[#fafafa] pb-20">
      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <form action={logoutStudent}>
          <button type="submit" className="flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-slate-700 transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-10">
        ...
        <div className="mb-10">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-indigo-50 border border-indigo-100 mt-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-slate-200/60 rounded-[2rem] p-6 md:p-8 shadow-sm">
              ...
              <div className="space-y-3">
                {materials && materials.length > 0 ? materials.map((doc) => (
                  <a key={doc.id} href={doc.pdf_url} target="_blank" rel="noreferrer" className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group">
                  ...
                  <div className="text-center py-8 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                    <p className="text-sm font-bold text-slate-400">No new materials uploaded yet.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200/60 rounded-[2rem] p-6 shadow-sm">
              ...
              <div className="space-y-3">
                {tests && tests.length > 0 ? tests.map((test) => (
                  <div key={test.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center">
                  ...
                  <p className="text-xs font-bold text-slate-400 text-center py-4">No tests recorded.</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-[2rem] p-6 text-white shadow-xl shadow-slate-900/10">
              ...
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-4">This Month</p>
              <div className="flex items-end gap-2 text-4xl font-black">
